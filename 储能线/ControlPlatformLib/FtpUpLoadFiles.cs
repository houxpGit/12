...
namespace ControlPlatformLib
{
    ...
    {
        public static string FTPCONSTR = "";//FTP的服务器地址，格式为ftp://192.168.1.234:8021/。ip地址和端口换成自己的，这些建议写在配置文件中，方便修改
        ...
        public static bool UploadFile(string path, string name)
        {
            ...
            try
            {
                ...
                path = path.Replace("\\", "/");
                path = FTPCONSTR + "/Path";//这个路径是我要传到ftp目录下的这个目录下
        ...
        public static void Onload(string file)
        {
            ...
            ftp = (FtpWebRequest)FtpWebRequest.Create(new Uri("ftp://169.254.39.4/" + f.Name));
            ftp.Credentials = new NetworkCredential();
            ftp.KeepAlive = false;
            ftp.Method = WebRequestMethods.Ftp.UploadFile;
            ftp.UseBinary = true;
            ftp.ContentLength = f.Length;
            int bufferLengh = 20480;
            byte[] buff = new byte[bufferLengh];
            int contentLen;
            try
            {
                FileStream fs = f.OpenRead();
                Stream sw = ftp.GetRequestStream();
                contentLen = fs.Read(buff, 0, bufferLengh);
                while (contentLen != 0)
                {
